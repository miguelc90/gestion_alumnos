...
def data_base_existe():
    try:
        ...
        #si la conexión es éxitosa retornamos True
        #esto significa que la base de datos existe
        return True
    except mysql.connector.Error as e:
        #si la base de datos no existe retornamos False
        if e.errno == errorcode.ER_BAD_DB_ERROR:
            return False
        else:
            #para cualquier otro error retornamos False
            #y mostramos el mensaje de error en pantalla
            print(f"Error: {e}")
            return False
...
def actualizar_Alumnos(cursor, id_alumno, parametro):
    cursor.execute("SELECT COUNT(*) FROM gestion_alumnos.alumnos WHERE id = %s AND activo = TRUE", (id_alumno,))
    ...
    query += " WHERE id = %s AND activo = TRUE"
    
    return query, nuevo_valor, id_alumno


def actualizar_datos_alumnos(cursor, id_alumno, parametro):
    cursor.execute("SELECT COUNT(*) FROM gestion_alumnos.alumnos WHERE id = %s AND activo = TRUE", (id_alumno,))
    if cursor.fetchone()[0] == 0:
        input("Pulse ENTER para continuar...")
        return None, None, None
...
def actualizar_por_campos(tabla, id_alumno, parametro):
    ...
    try:
        ...
        if tabla == 1:
            query, nuevo_valor, id_alumno = actualizar_Alumnos(cursor, id_alumno, parametro)
        elif tabla == 2:
            query, nuevo_valor, id_alumno = actualizar_datos_alumnos(cursor, id_alumno, parametro)
        else:
        ...
        print(f"Error, no se ha podido actualizar el registro: {e}")
        input("Pulse ENTER para continuar...")
        os.system('cls')
    finally:
        cursor.close()
...
def opcion_actualizacion_completa(id_alumno):
    ...
    try:
        ...
        cursor.execute("SELECT * FROM gestion_alumnos.alumnos WHERE id = %s AND activo = TRUE", (id_alumno,))
        registro_alumnos = cursor.fetchone()

        cursor.execute("SELECT * FROM gestion_alumnos.datos_de_alumnos WHERE id = %s AND activo = TRUE", (id_alumno,))
...
def mostrar_coincidencia_id(cursor, id_alumno):
    cursor.execute("SELECT * FROM gestion_alumnos.alumnos a join gestion_alumnos.datos_de_alumnos d on a.id=d.id_alumno where a.id=%s AND a.activo = TRUE AND d.activo = TRUE",(id_alumno,))
    mostrar=cursor.fetchone()
    if mostrar:
        print(f"\n{"id":<3} {"Nombre":<15} {"Apellido":<15} {"DNI":<10} {"Domicilio":20} {"Telefono":<13} {"Fecha de nacimiento"}")
        print(f"{mostrar[0]:<3} {mostrar[1]:<15} {mostrar[2]:<15} {mostrar[3]:<10} {mostrar[7]:<20} {mostrar[8]:<13} {mostrar[9]}")
        print()
        return True
    else:
        print(f"El id solcitado ({id_alumno}) no existe")
        return False
...
def eliminar(id_alumno):
    ...
    try:
        ...
        if not resultado:
            ...
            resultado_coincidencia = mostrar_coincidencia_id(cursor, id_alumno)
            if resultado_coincidencia:
                ...
                input("Pulse ENTER para continuar...2")
                os.system("cls")
                return
    ...
    try:
        ...
        match consulta:
            case 1:
                ...
                cursor.execute('SELECT * FROM gestion_alumnos.alumnos a join gestion_alumnos.datos_de_alumnos d on a.id=d.id_alumno WHERE a.activo = TRUE AND d.activo = TRUE')
                resultado=cursor.fetchall()
                print(f"{"id":<3} {"Nombre":<21} {"Apellido":<21} {"DNI":<9} {"Domicilio":31} {"Telefono":<13} {"Fecha de nacimiento"}")
                print()
                for fila in resultado:
                    print(f"{fila[0]:<3} {fila[1].title():<21} {fila[2].title():<21} {fila[3]:<9} {fila[7].title():<31} {fila[8]:<13} {fila[9]}") 
            case 2:
                # SI LA OPCIÓN ES 2 SE CONSULTA SOLO LOS DATOS DE LA TABLA ALUMNOS
                cursor.execute('SELECT * FROM gestion_alumnos.alumnos WHERE activo = TRUE')
                resultado=cursor.fetchall()
            ...
            case 3:
                ...
                cursor.execute('SELECT * FROM gestion_alumnos.datos_de_alumnos WHERE activo = TRUE')
                resultado=cursor.fetchall()
...
def ordenar_datos(opcion_orden):
    ...
    #le asignamos a la variable globarl el valor validado de opcion_orden
    ...
    match opcion_orden:
        ...
        case _:
            ...
            return None
    #ejecutamos la query del caso correspondiente
    cursor.execute(query)
    #en la variable resultados almacenamos todas las filas que nos trae el cursor.fetchall()
    resultados = cursor.fetchall()
    #en la variable headers almacenamos los nombres de las columnas 
    #iternando con un for para extraer dichos nombres
...
def menu_de_ordenamiento():
    try:
        #creamos las vistas llamando a la funcion crear_vistas()
        crear_vistas()
        #creamos una lista con las opciones validas que usaremos para el menú
        ...
        while True:
            ...
            #si opcion se encuentra en la lista opciones_validas
            #entra en el bloque if
            if opcion in opciones_validas:
                #invocamos a la funcion ordenar_datos y le pasamos como argumento
                #la opcion ingreseada
                ordenar_datos(opcion)
                #en resultados almacenamos los datos de las filas
                #en headers almacenamos los nombres de las columnas
                #dependiendo el metodo de ordenamiento que se haya elegido
                #a través de la funcion ordenar_datos
                resultados, headers = ordenar_datos(opcion)
                #si hay datos que mostrar
                if resultados:
                    #se mostrará una tabla en formato de grilla con los datos
                    ...
                    #si no hay datos para mostrar se mostrará por mensaje los pasos a seguir
                    #si es que se desea visualizar algun método de ordenamiento
                    print("Debe ingresar registros para poder ordenarlos(menú principal>opción 2)")
                    input("Puse ENTER para continuar...")
                    os.system("cls")
                    return
            else:
                #si la opción no se encuentra en la lista de opciones_validas
                #se mostrará el mensaje de advertencia
                print("Debe seleccionar una de las opciones ofrecidas\n")
    #bloque except para capturar el error de tablas o culumnas que no existen
    except mysql.connector.ProgrammingError as e:
        #se mostrará un mensaje con los pasos a seguir para poder utilizar las funcionalidades
